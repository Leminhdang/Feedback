...
const AdminLogin = () => {
  ...
  const ham = async () => {
    const response = await fetch("http://localhost:8080/login/loginTeacher", {
      method: "POST",
      mode: "cors",
      headers: {
    ...
    if (res.token) {
      ...
      cookies.set("CN", res.formteacher);
      setToken(res.token);
      cookies.set("fullname", res.fullname);
      console.log("namaaaaaaaaaa", res.fullname);
  ...
  return (
    <MediaQuery minDeviceWidth={1224} device={{ deviceWidth: 1600 }}>
      ...
      {token ? (
        <Redirect from="/teacher/login" to="/teacher/index" />
      ) : (
